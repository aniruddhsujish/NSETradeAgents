...
- [x] Sentiment agent — ReAct research agent (Tavily) + Claude Haiku scoring, sector-aware queries
- [x] Risk agent — deterministic 3 trade gates + position sizing (25% of starting capital per position)
- [x] Decision agent — Claude Sonnet 6-step framework, BUY/HOLD/SELL with confidence score
...
- [ ] Fundamental agent — revenue growth, D/E ratio, ROE, promoter holding pre-filter before LLM pipeline
...
- [ ] ATR-based stop loss — `price - 2×ATR` replaces fixed 7%; adapts to each stock's volatility
- [ ] Cash buffer — enforce 20-30% uninvested at all times; currently 4×25% = 100% deployed
...
- [ ] Raise `min_confidence` to 0.75 once baseline performance is established

## V2 — Screener

- [ ] Store sector from NSE CSV — use for sector exposure tracking and macro filtering
- [ ] Fundamental pre-filter — P/E, revenue growth, debt levels before yfinance download
...
- [ ] Backtesting module — run strategy on historical OHLCV data
