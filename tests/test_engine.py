...
def stub_engine(monkeypatch):
    ...
    def install(bars: dict, *, signals_on: set[date] | None = None, atr_pct: float = 3.0):
        ...
        monkeypatch.setattr(engine, "compute_indicators", lambda df: {
            "current_price": float(df["Close"].iloc[-1]), "atr_pct": atr_pct,
        })
        monkeypatch.setattr(engine, "_compute_signal", lambda ind: "BUY")
        monkeypatch.setattr(engine, "evaluate_candidate",
                            lambda ind: ({"screener_score": 1.0}, "passed"))
        monkeypatch.setattr(engine, "compute_rules_confidence",
                            lambda *a, **k: {"score": 99})
...
def test_a_position_opened_at_the_close_cannot_exit_the_same_day(stub_engine):
    ...
    assert 100.0 * 0.60 < entry * (1 - 2.5 * 3.0 / 100), (
