...
def _yearly_breakdown(trades: list[ClosedTrade], equity_curve: list[tuple[date, float]]):
    ...
    for y in all_years:
        ...
        pf = (
            sum(t.pnl for t in wins) / abs(gross_loss)
...
def print_report(
    ...
    gross_loss = sum(t.pnl for t in losses)
    profit_factor = (
        sum(t.pnl for t in wins) / abs(gross_loss) if gross_loss != 0 else float("inf")
    )
    avg_hold = (
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "exit_price", "shares", "pnl", "pnl_pct", "exit_reason", "score",
            ],
        )
        writer.writeheader()
        for t in trades:
